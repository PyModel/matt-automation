...
2. **Per-worktree recipe, run-scoped.** Ports = `3000 + 100 * run_id + NN`; database/schema suffix `_<slug>_t<NN>`; compose project name `<slug>-t<NN>`; temp dirs under the worktree; `.env` copied with the substitutions applied. Every path the recipe or the install step creates in a worktree that the repo does not already ignore (`git check-ignore`) goes into `$(git rev-parse --git-common-dir)/info/exclude`, which every worktree shares and which is never a tracked change, so an implementer can leave a clean tree (CONTRACT.md § The receipt check). Any resource with no override → `max_concurrent_tickets: 1`, logged why.
...
5. **Worker.** `worker: subagent` unless the objective or `/kun` names `pi[/<provider>/<model>]` (BUILD.md § Implementer backend). `tiers:` maps capability pairs to models (CONTRACT.md § Capability) only when the objective names models or the probe shows the worker takes a model choice; else `tiers: none`.
