...
Load implement-spec by path and run it as the orchestrator with the bindings below. Read PIPELINE.md § Isolation and § Budgets and CONTRACT.md first. `node ROOT/scripts/goal.mjs registry <slug> building`. Record each ticket's starting point `<ticket-base>` on dispatch; the whole deliverable is diffed against `review_base` (BOOTSTRAP.md § 0c). Done per PIPELINE.md § Completion criteria per stage (7).
...
- **Step 4, implementers:** one per frontier ticket, each in its own worktree and branch (PIPELINE.md § Isolation), dispatched only while the budgets allow (PIPELINE.md § Budgets) and `STOP` is absent. Local tracker: take tickets with `node ROOT/scripts/goal.mjs take <feature> <free slots>`, which reads the frontier and flips the tickets to `in-flight` in one locked transaction, so two orchestrators never take one ticket. GitHub tracker: read the frontier and set the issue's status inside `goal.mjs with-lock frontier-<feature> -- <gh commands>`. Parallel only when claims are disjoint (stage 6b guarantees this for `ready-for-agent` tickets). Per taken ticket, before dispatch: create its worktree, re-check CONTRACT.md § Readiness, compile its contract to `tickets/<NN>.goal.md` (CONTRACT.md § The contract), and pick its worker from its capability (`goal.mjs frontier` reports it; NOW `tiers:` maps it). Record each dispatch in NOW and Events.
- **Step 5, merger subagent:** per ticket back from its implementer, the orchestrator saves the implementer's final message to `tickets/<NN>.receipt.md`; then, as one `goal.mjs with-lock integration -- <merge script>` transaction:
  0. CONTRACT.md § The receipt check. A refused receipt, or any conclusion but `completed`, stops here: the ticket goes to § Stuck detection with the check's `problems` or the receipt's `blockers`.
...
The brief is the contract file `tickets/<NN>.goal.md` (CONTRACT.md § The contract), plus, verbatim: the implement file's contents, the guards from PITFALLS.md, this list of rules, the receipt format (CONTRACT.md § The receipt), and LEDGER.md § Subagent contract last. State the ledger holds stays in the ledger; the contract points at it.
...
Before the first dispatch write `$d/accepted/scout.md` and `$d/accepted/research.md` citing `findings.md`, `spec.md` and the notes dir: stages 1–5 did that work. The brief file is the contract plus the rest of § The implementer brief; `--model` comes from NOW `tiers:` for the ticket's capability, else NOW `worker:`. Poll with `--poll --task impl --run-dir "$d"`. `COMPLIANT` hands the ticket to step 5, whose receipt is the worker's final message (CONTRACT.md § The receipt); any other `orcStatus` is a stuck signal for § Stuck detection, with the status file's `reason` appended to the re-dispatch brief. `COMPLIANT` proves the dispatch, never the ticket: only the receipt check does. One run directory per ticket, so the worker, cycles and budget are per ticket.

## Stuck detection

Separate heartbeat liveness, active command execution, and completed-slice progress. A dispatched ticket is **stuck** when any of: its receipt is refused or its conclusion is `partial`, `blocked`, `stuck`, or `claims-breach`; its `heartbeat` is older than 30 minutes and its `agent` is not alive; its `slice` counter is unchanged across 3 heartbeats while no command is running; its status file reports the same non-quarantined test failing 5 times; the per-ticket slice or wall-clock cap in NOW `budgets:` is reached (`stuck: too big`, the lever is upstream: it should have been split); or the harness reports the agent gone.
