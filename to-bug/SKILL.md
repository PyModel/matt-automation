...
- **Stage 8 review** always runs, even on a one-line fix: `code-review` Standards axis plus `defensive-design` Review mode over the diff; the Spec axis reads `spec.md` (the post-mortem). Findings fixed by the single fix subagent as usual.
