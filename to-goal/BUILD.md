...
- **Step 5, merger subagent:** per returned ticket: under the `integration` lock, rebase the ticket branch onto the run head; for `shared-regenerate` conflicts run the regenerate command and take the result; other conflicts → `resolving-merge-conflicts`, and when any conflict was resolved re-run the suite and a single-agent review of the resolved hunks before merging. Then `merge --ff-only`, record the ticket as the range `<before>..<after>` on the run branch (pre-rebase SHAs in status files are historical only), remove the worktree and branch. Before merging, diff the ticket's touched paths against its claims: an undeclared `exclusive` path or an unapproved `guarded` path is a **claims breach**: the merger rejects the ticket, the orchestrator logs it, and either re-dispatches with the claim added (if no other ticket claims that path) or splits the offending change into a new ticket.
- **Step 6, frontier:** after every merge the orchestrator ticks the ticket, sets `**Status:** done` on the tracker, updates `todo.md` and the ledger, commits the control plane under the `control` lock, and in flat mode runs `code-review` for that ticket's range `<ticket-base>...HEAD` itself; then recomputes the frontier under the lock and dispatches what was unblocked.
...
8. Heartbeat: update `tickets/<NN>.status.md` before and after every slice and before any command expected to run longer than a minute (install, suite) with `heartbeat: <time>`, `agent: <id or pid>`, `active_cmd: <name if active>`, `slice: n/m`, last commit, suite result; commit the status file to the control plane under the `control` lock. Redact secrets first. Spawn no agents; load only `tdd`, `defensive-design`, `diagnosing-bugs`, `code-review`, `wizard`, by path (SKILL.md § Loading skills).
