...
8. Heartbeat: update `tickets/<NN>.status.md` before and after every slice and before any command expected to run longer than a minute (install, suite) with `heartbeat: <time>`, `agent: <id or pid>`, `active_cmd: <name if active>`, `slice: n/m`, last commit, suite result; commit the status file to the control plane under the `control` lock. Redact secrets first. Spawn no agents except `tdd`, `defensive-design`, `diagnosing-bugs`, `code-review`, `wizard` via the skill tool.
