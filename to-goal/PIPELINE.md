...
- `<objective-slug>` comes from `goal.mjs slug` (CONTROL.md § Run registry).
...
- **Tickets + claims**: one file per ticket; each has a demo path, tier, claims, "Blocked by"; acceptance criteria for new behavior confirmed red at base (invariants remain green); no cycles; no two unordered open tickets share an `exclusive` claim; `ready-for-agent` stripped from the parent spec; ticket table in `todo.md`.
