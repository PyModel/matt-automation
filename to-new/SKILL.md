...
- **Stage 1 route is pinned.** ask-matt is still read at stage 00; its on-ramp for a greenfield project too big for one session is `/wayfinder`, which is this route. Log `route: On-ramps → wayfinder (greenfield, pinned by /to-new)`, research need `up-front`. Repo exploration covers only what exists (a monorepo's conventions, an existing skills hub's layout, the harness's installed-skills directory for a skill).
