...
`/to-new` is `/to-goal` for a target that does not exist yet. Let `ROOT` be the parent of this folder's real path (`realpath`, as in to-goal SKILL.md § Loading skills). Read `ROOT/to-goal/SKILL.md` and follow it verbatim, phase files included, with the overrides below. `/to-goal` itself switches to these overrides when stage 00 finds no repository or stage 1 classifies an objective `greenfield`.
